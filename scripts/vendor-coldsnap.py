#!/usr/bin/env python3
"""Import and verify the commit-pinned first-party ColdSnap plugin snapshot."""
...
LOCK_PATH = ROOT / "vendor" / "coldsnap.lock"
SOURCE_DESTINATION = ROOT / "src" / "sparkrun" / "plugins" / "coldsnap"
TEST_DESTINATION = ROOT / "tests" / "vendor" / "coldsnap"
PROVENANCE_PATH = SOURCE_DESTINATION / "VENDORED.toml"
_IGNORED_PARTS = {"__pycache__", ".pytest_cache", ".ruff_cache"}
_UPSTREAM_SOURCE = "src/sparkrun/plugins/coldsnap"
_UPSTREAM_TESTS = "tests/test_coldsnap_*.py"
UPSTREAM_REPOSITORY = "https://github.com/sparksq/sparkrun-coldsnap-plugin.git"
UPSTREAM_SLUG = "sparksq/sparkrun-coldsnap-plugin"
...
def latest_release_tag() -> str:
    ...
    try:
        ...
        raise VendorError(
            "could not resolve the latest ColdSnap plugin release; install/authenticate GitHub CLI "
...
def _repository(source: str) -> Iterator[Path]:
    ...
    with tempfile.TemporaryDirectory(prefix="sparkrun-coldsnap-source-") as temporary:
        ...
        try:
            ...
            raise VendorError("could not clone ColdSnap plugin source %s" % source) from error
        yield repository
...
def _manifest(repository: Path, commit: str) -> PluginManifest:
    ...
    if values["name"] != "coldsnap" or values["module"] != "sparkrun.plugins.coldsnap":
        raise VendorError("upstream manifest does not describe the in-tree ColdSnap module")
    if values["feature"] != "plugins.coldsnap":
        raise VendorError("upstream manifest does not use the plugins.coldsnap feature gate")
...
def _render_lock(
    *,
    ...
    lines = [
        "# Generated by scripts/vendor-coldsnap.py; do not edit by hand.",
        "schema = 1",
...
def _check_destinations() -> None:
    # Refuse redirected destination roots before reading or replacing anything.
    for directory in (SOURCE_DESTINATION, TEST_DESTINATION):
        if directory.exists() and not directory.is_dir():
            raise VendorError("vendor destination must be a directory: %s" % directory)
    for destination in (LOCK_PATH, SOURCE_DESTINATION, TEST_DESTINATION):
        for path in (destination, *destination.parents):
            if path == ROOT:
                break
            if path.is_symlink():
                raise VendorError("vendor destination may not contain symlinks: %s" % path)


def _validate_host_contract(manifest: PluginManifest, source: Path) -> None:
    """Check metadata without importing or executing upstream plugin code."""
    host = tomllib.loads((ROOT / "pyproject.toml").read_text())["project"]["version"]

    def version(value: str) -> tuple[int, ...]:
        if not re.fullmatch(r"[0-9]+\.[0-9]+(?:\.[0-9]+)?", value):
            raise VendorError("vendoring requires a numeric host version and comparison bounds: %r" % value)
        parts = tuple(map(int, value.split(".")))
        return parts + (0,) * (3 - len(parts))

    comparisons = {"<": operator.lt, "<=": operator.le, ">": operator.gt, ">=": operator.ge, "==": operator.eq, "!=": operator.ne}
    for clause in manifest.sparkrun.split(","):
        bound = re.fullmatch(r"(<=|>=|==|!=|<|>)\s*([0-9]+\.[0-9]+(?:\.[0-9]+)?)", clause.strip())
        if bound is None:
            raise VendorError("unsupported upstream host requirement: %r" % manifest.sparkrun)
        if not comparisons[bound[1]](version(host), version(bound[2])):
            raise VendorError("plugin requires sparkrun %s; host is %s" % (manifest.sparkrun, host))

    try:
        module = ast.parse((source / "__init__.py").read_text())
    except (OSError, SyntaxError) as error:
        raise VendorError("could not read the plugin API declaration: %s" % error) from error
    declarations = []
    for statement in module.body:
        if isinstance(statement, ast.Assign):
            ...
            continue
        if any(isinstance(target, ast.Name) and target.id == "SPARKRUN_PLUGIN_API_VERSION" for target in targets):
            value = statement.value
            declarations.append(value.value if isinstance(value, ast.Constant) else None)
    if len(declarations) != 1 or type(declarations[0]) is not int or declarations[0] != 1:
        raise VendorError("upstream plugin must declare the literal SPARKRUN_PLUGIN_API_VERSION = 1 for this host")


def verify(*, allow_absent: bool = False) -> None:
    _check_destinations()
    if not LOCK_PATH.exists() and not _regular_files(SOURCE_DESTINATION) and not _regular_files(TEST_DESTINATION):
        if allow_absent:
            print("ColdSnap is not vendored; optional snapshot verification skipped")
            return
    lock = _load_lock()
    for name, wanted in (("name", "coldsnap"), ("module", "sparkrun.plugins.coldsnap"), ("feature", "plugins.coldsnap")):
        if lock.get(name) != wanted:
            raise VendorError("vendor lock does not describe ColdSnap: %s" % name)
    if not lock["files"]:
        raise VendorError("vendor snapshot must not be empty")
    ...
    try:
        ...
        raise VendorError("could not read packaged ColdSnap provenance: %s" % error) from error
    for name in ("repository", "commit", "tree", "version", "content_sha256", "release_tag", "application_profile_api"):
        if provenance.get(name) != lock.get(name):
            raise VendorError("packaged ColdSnap provenance disagrees with the lock for %s" % name)

    print("ColdSnap vendor snapshot verified: %s at %s" % (lock["version"], lock["commit"]))


def update(*, source: str, revision: str, initial: bool, force: bool, release_tag: str | None = None) -> None:
    _check_destinations()
    if release_tag is None and not _is_hex_digest(revision, 40):
        raise VendorError("--rev requires a full lowercase commit SHA; use --latest for a published release")
    if not LOCK_PATH.exists() and (_regular_files(SOURCE_DESTINATION) or _regular_files(TEST_DESTINATION)):
        raise VendorError("unlocked ColdSnap content already exists; refusing an initial import")
    if LOCK_PATH.exists():
        if initial:
    ...
    with _repository(source) as repository:
        ...
        with tempfile.TemporaryDirectory(prefix="sparkrun-coldsnap-import-") as temporary:
            ...
            if Path("VENDORED.toml") in source_mapping:
                raise VendorError("upstream source may not provide the host-owned VENDORED.toml")
            _validate_host_contract(manifest, upstream_source)
...
def _parser() -> argparse.ArgumentParser:
    ...
    verify_parser = commands.add_parser("verify", help="verify the checked-in snapshot without network access")
    verify_parser.add_argument("--allow-absent", action="store_true", help="accept a completely absent optional integration")
    ...
    revision.add_argument("--rev", help="full reviewed upstream commit SHA to pin")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        if args.command == "verify":
            verify(allow_absent=args.allow_absent)
        else:
